/*
 ...
 SSD1306 display driver (TWI mode)
...
{
  while(i2c_write(SSD1306_TWI_DATA));
  i2c_write(byte);
}
...
{
  i2c_write(SSD1306_TWI_INSTRUCTION);
  i2c_write(byte);
}
...
ssd1306_init_display (void)
{
  ...
  i2c_start_wait(SSD1306_TWI_ADDRESS << 1);
