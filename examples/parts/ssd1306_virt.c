...
ssd1306_update_command_register (ssd1306_t *part)
{
	...
	switch (part->spi_data)
	{
		...
		case SSD1306_VIRT_SET_CONTRAST:
			...
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: CONTRAST SET COMMAND: 0x%02x\n", part->spi_data);
			return;
		case SSD1306_VIRT_DISP_NORMAL:
			ssd1306_set_flag (part, SSD1306_FLAG_DISPLAY_INVERTED,
			                  0);
			ssd1306_set_flag (part, SSD1306_FLAG_DIRTY, 1);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: DISPLAY NORMAL\n");
		...
		case SSD1306_VIRT_DISP_INVERTED:
			ssd1306_set_flag (part, SSD1306_FLAG_DISPLAY_INVERTED,
			                  1);
			ssd1306_set_flag (part, SSD1306_FLAG_DIRTY, 1);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: DISPLAY INVERTED\n");
			SSD1306_CLEAR_COMMAND_REG(part);
			return;
		case SSD1306_VIRT_DISP_SUSPEND:
			ssd1306_set_flag (part, SSD1306_FLAG_DISPLAY_ON, 0);
			ssd1306_set_flag (part, SSD1306_FLAG_DIRTY, 1);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: DISPLAY SUSPENDED\n");
			SSD1306_CLEAR_COMMAND_REG(part);
			return;
		case SSD1306_VIRT_DISP_ON:
			ssd1306_set_flag (part, SSD1306_FLAG_DISPLAY_ON, 1);
			ssd1306_set_flag (part, SSD1306_FLAG_DIRTY, 1);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: DISPLAY ON\n");
			SSD1306_CLEAR_COMMAND_REG(part);
			return;
		case SSD1306_VIRT_SET_PAGE_START_ADDR
			...
			part->cursor.page = part->spi_data
			                - SSD1306_VIRT_SET_PAGE_START_ADDR;
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: SET PAGE ADDRESS: 0x%02x\n", part->spi_data);
			SSD1306_CLEAR_COMMAND_REG(part);
			return;
		...
		case SSD1306_VIRT_SET_SEG_REMAP_0:
			ssd1306_set_flag (part, SSD1306_FLAG_SEGMENT_REMAP_0,
			                  1);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: SET COLUMN ADDRESS 0 TO OLED SEG0 to \n");
			SSD1306_CLEAR_COMMAND_REG(part);
			return;
		case SSD1306_VIRT_SET_SEG_REMAP_127:
			ssd1306_set_flag (part, SSD1306_FLAG_SEGMENT_REMAP_0,
			                  0);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: SET COLUMN ADDRESS 127 TO OLED SEG0 to \n");
			SSD1306_CLEAR_COMMAND_REG(part);
			return;
		case SSD1306_VIRT_SET_COM_SCAN_NORMAL:
			ssd1306_set_flag (part, SSD1306_FLAG_COM_SCAN_NORMAL,
			                  1);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: SET COM OUTPUT SCAN DIRECTION NORMAL \n");
			SSD1306_CLEAR_COMMAND_REG(part);
			return;
		case SSD1306_VIRT_SET_COM_SCAN_INVERTED:
			ssd1306_set_flag (part, SSD1306_FLAG_COM_SCAN_NORMAL,
			                  0);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: SET COM OUTPUT SCAN DIRECTION REMAPPED \n");
			SSD1306_CLEAR_COMMAND_REG(part);
			return;
...
ssd1306_update_setting (ssd1306_t *part)
{
	switch (part->command_register)
	{
		case SSD1306_VIRT_SET_CONTRAST:
			...
			ssd1306_set_flag (part, SSD1306_FLAG_DIRTY, 1);
			SSD1306_CLEAR_COMMAND_REG(part);
			parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: CONTRAST SET: 0x%02x\n", part->contrast_register);
			return;
		case SSD1306_VIRT_SET_PAGE_ADDR:
...
ssd1306_cs_hook (struct avr_irq_t * irq, uint32_t value, void * param)
{
	ssd1306_t * p = (ssd1306_t*) param;
	p->cs_pin = value & 0xFF;
	parts_global_logger(&p->logger,LOG_DEBUG,"SSD1306: CHIP SELECT:  0x%02x\n", value);
...
ssd1306_di_hook (struct avr_irq_t * irq, uint32_t value, void * param)
{
	...
	parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: DATA / INSTRUCTION:  0x%08x\n", value);
}
...
{
	ssd1306_t * part = (ssd1306_t*) param;
	parts_global_logger(&part->logger,LOG_DEBUG,"SSD1306: RESET\n");
	if (irq->value && !value)
	{
		// Falling edge
...
static const char * irq_names[IRQ_SSD1306_COUNT] = {
	 [IRQ_SSD1306_SPI_BYTE_IN] = "=ssd1306.spi.in",
	 [IRQ_SSD1306_RESET] = "<ssd1306.input.rs",
	 [IRQ_SSD1306_DATA_INSTRUCTION] = "<ssd1306.input.rw",
	 [IRQ_SSD1306_ENABLE] = "<ssd1306.input.e",
     [IRQ_SSD1306_ADDR] = "7>hd44780.trace.ADDR"
};
...
ssd1306_init (struct avr_t *avr, struct ssd1306_t * part, int width, int height)
{
	...
	ssd1306_initialize(&avr->irq_pool,&avr->cycle_timers,part,width,height);
}
...
ssd1306_initialize ( avr_irq_pool_t *irq_pool, avr_cycle_timer_pool_t * cycle_timers,struct ssd1306_t * part, int width, int height)
{
	 ...
	 */
	part->irq = avr_alloc_irq (irq_pool, 0, IRQ_SSD1306_COUNT,
    	                       irq_names);
	...
	parts_global_logger(&part->logger,LOG_OUTPUT,"SSD1306: %duS is %d cycles for your AVR\n", 37,
	        (int) avr_usec_to_cycles (cycle_timers->clock, 37));
	parts_global_logger(&part->logger,LOG_OUTPUT,"SSD1306: %duS is %d cycles for your AVR\n", 1,
	        (int) avr_usec_to_cycles (cycle_timers->clock, 1));
}
