...
jit_generate(opcode, 0+F_JUMP+F_OP,
"get_vd5_vr5(opcode);\n"
"uint16_t res = vd == vr;\n"
...
jit_generate(opcode, 0+F_JUMP+F_OP+F_NO_PC,
"if (opcode == 0x9518) {\n"
"SREG_FLUSH(\n"
...
"_sreg = _avr_set_r(avr, _sreg, 0, avr_flash[z]);\n"
				...
				case 0x95d8: {	// ELPM -- Load Program Memory R0 <- (Z) -- 1001 0101 1101 1000
					if (!avr_rampz)
					 ...
					 {
jit_generate(opcode, 0,
"uint32_t z = avr_data[R_ZL] | (avr_data[R_ZH] << 8) | (avr_data[avr_rampz] << 16);\n"
